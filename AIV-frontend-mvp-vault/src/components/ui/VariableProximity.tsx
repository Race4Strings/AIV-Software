...
const VariableProximity = forwardRef<HTMLSpanElement, VariableProximityProps>((props, ref) => {
    ...
    useAnimationFrame(() => {
        ...
        letterRefs.current.forEach((letterRef, index) => {
            // Filter out null refs and ensure we don't crash
            if (!letterRef) return;

            // Safety check if interpolatedSettingsRef is not yet initialized for this index
            if (!interpolatedSettingsRef.current[index]) {
    ...
    return (
        <span
            ref={ref}
            className={`${className} variable-proximity inline-block`}
            onClick={onClick}
            style={{ display: 'inline', overflow: 'visible', lineHeight: 1.3, ...style }}
            {...restProps}
        >
